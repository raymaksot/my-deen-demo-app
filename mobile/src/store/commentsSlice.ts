...
export const createComment = createAsyncThunk(
  'comments/create',
  async (payload: { parentType: ParentType; parentId: string; text: string }) => {
    const res = await commentsService.create(payload);
    return res;
  }
);
...
const slice = createSlice({
  ...
  initialState,
  reducers: {
    hydrate(state, action: PayloadAction<CommentsState>) {
...
export const { hydrate, addLocalComment } = slice.actions;
